    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-3 mb-8">
                <a href="{{ route('books.create') }}" class="btn btn-error bg-[#B8292D] text-white w-fit">Tambah Buku</a>
                {{-- <input type="text" placeholder="Type here" class="input input-bordered w-full max-w-xs">
                 --}}
                <form action="{{ route('books.search') }}"
                    class="group/search-bar p-[14px_18px] bg-white shadow-2xl text-black ring-1 ring-[#414141] hover:ring-[#888888] max-w-[560px] w-full rounded-full transition-all duration-300">
                    <div class="relative text-left">
                        ...
                        <input type="text" id="searchInput" name="search"
                            class="bg-white shadow-2xl text-black w-full pl-[36px] focus:outline-none placeholder:text-[#595959] pr-9 border-none outline-none"
            ...
            <div class="grid grid-cols-5 mb-8 gap-4">
                @foreach ($categories as $category)
                    <a href="{{ route('books.category', $category) }}"
                        class="flex ard bg-white w-full h-fit shadow-xl col-span-1 rounded-xl justify-center">


                        {{-- <img class="p-4  rounded-t-lg" src="storage/" alt="Shoes" /> --}}

                        <p class="text-black p-4">{{ $category->name }}</p>
            ...
            <div class="flex flex-row flex-wrap gap-8 justify-center">
                @foreach ($books as $book)
                    <a href="{{ route('books.show', $book->id) }}"
                        class="card bg-white w-96 shadow-xl basis-1/5 rounded-xl ">


                        <img class="p-4 h-72 rounded-t-lg" src="storage/{{ $book->cover }}" alt="Shoes" />

                        <div class="card-body mb-4">
                            <h2 class="card-title text-black">{{ $book->title }}</h2>
